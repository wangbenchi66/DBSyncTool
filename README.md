...
| 版本包 | 适用场景 | 说明 | 下载入口 |
| :--- | :--- | :--- | :--- |
| **桌面端（推荐）** | 所有用户 | win-x64 自包含单文件，无需安装 .NET | [⬇️ 下载 DBSync.Desktop](https://github.com/wangbenchi66/DBSyncTool/releases) |
| **CLI 命令行** | 服务器 / 自动化 | win-x64 自包含，适合无图形界面环境 | [⬇️ 下载 dbsync CLI](https://github.com/wangbenchi66/DBSyncTool/releases) |
| **源码运行** | 开发者 | 需 .NET 10 SDK | 见下方[本地构建](#build) |

### 基础使用流程：

1. 下载桌面端 zip，**整包解压**后运行 `DBSync.Desktop.exe`；
