...
public sealed class PostgresSqlGenerator : ISqlGenerator
{
    ...
    public string GenerateDropTable(TableModel table) => GenerateDropTable(DatabaseType.PostgreSql, table);

    public IReadOnlyList<string> GenerateAlterTable(TableDiff diff) => GenerateAlterTable(DatabaseType.PostgreSql, diff);
        ...
        IReadOnlyList<IReadOnlyDictionary<string, string?>> rows)
    {
        ...
        return rows.Select(row => BuildInsertStatement(insertPrefix, columns, row)).ToList();
    }
    ...
    {
        var identity = includeIdentity && (column.IsIdentity || column.IsAutoIncrement)
    ...
    private static string FormatColumnType(ColumnModel column)
    {
        ...
        if (lowerTypeName is "char" or "nchar" or "varchar" or "nvarchar" or "character varying" or "character" or "bit" or "varbit")
    ...
    {
        return DbDialectSupport.QuoteTableName(DatabaseType.PostgreSql, schema, tableName);
    }
        ...
        IReadOnlyDictionary<string, string?> row)
    {
        ...
        return $"{insertPrefix} VALUES ({string.Join(", ", values)});";
    }
